...
std::string armorTypeName(ArmorType::ArmorType type)
{
    ...
    if (index < ArmorType::ArmorTypeStrings.size()) {
        return ArmorType::ArmorTypeStrings[index];
    }
    return "UNKNOWN";
}
...
{
    if (type == ArmorType::Nearest) {
        return "NEAREST";
    }
    if (type == ArmorType::Middle) {
        return "MIDDLE";
    }
...
TargetManager::TargetManager(std::shared_ptr<YAML::Node> config_file_ptr)
{
    ...
    max_temp_lost_frames_ =
        std::max(0, config["max_temp_lost_frames"].as<int>());
    outpost_max_temp_lost_frames_ =
        std::max(0, config["outpost_max_temp_lost_frames"].as<int>());
    enable_priority_switch_ = config["enable_priority_switch"].as<bool>();

    const YAML::Node reset_time_node =
        (*config_file_ptr)["reset_predictor_time"];
    if (reset_time_node) {
        const double reset_time_ms = reset_time_node.as<double>();
        if (std::isfinite(reset_time_ms) && reset_time_ms > 0.0) {
            max_frame_gap_s_ = reset_time_ms / 1000.0;
        }
    }
}
    ...
    ArmorType::ArmorType acquire_policy)
{
    ...
    if (status_.state == TargetState::LOST) {
        const ArmorResult* acquisition =
            selectAcquisition(measurements, frame_size, acquire_policy);
        if (acquisition == nullptr) {
            return update;
        }

        status_.target_type =
            static_cast<ArmorType::ArmorType>(acquisition->number);
    ...
    switch (status_.state) {
        case TargetState::DETECTING:
            if (!found) {
                ...
                if (status_.detect_count >= min_detect_frames_) {
                    status_.state = TargetState::TRACKING;
                }
            }
            break;
        ...
        case TargetState::TRACKING:
            if (!found) {
                status_.state = TargetState::TEMP_LOST;
                status_.temp_lost_count = 1;
            }
            break;
        ...
        case TargetState::TEMP_LOST:
            if (found) {
                ...
                if (status_.temp_lost_count > currentMaxTempLostFrames()) {
                    releaseTarget(update);
                }
            }
            break;
...
const TargetManagerStatus& TargetManager::status() const
{
    return status_;
}

void TargetManager::drawOverlay(
    cv::Mat& frame,
    const std::optional<std::string>& ekf_state) const
{
    ...
    cv::rectangle(frame, cv::Rect(10, 10, panel_width, panel_height),
                  cv::Scalar(20, 20, 20), cv::FILLED);

    const std::string target_name = status_.target_type.has_value()
        ? armorTypeName(*status_.target_type)
        : "NONE";
    const std::string policy_name = acquisitionPolicyName(status_.acquire_policy);

    cv::putText(frame, "TARGET STATE: " + std::string(stateName(status_.state)),
                cv::Point(20, 35), cv::FONT_HERSHEY_SIMPLEX, 0.68,
                color, 2, cv::LINE_AA);
    cv::putText(frame, "TARGET: " + target_name,
                cv::Point(20, 65), cv::FONT_HERSHEY_SIMPLEX, 0.68,
                color, 2, cv::LINE_AA);
    cv::putText(frame, "detect: " + std::to_string(status_.detect_count),
                cv::Point(20, 95), cv::FONT_HERSHEY_SIMPLEX, 0.62,
                color, 2, cv::LINE_AA);
    cv::putText(frame, "temp_lost: " + std::to_string(status_.temp_lost_count),
                cv::Point(20, 125), cv::FONT_HERSHEY_SIMPLEX, 0.62,
                color, 2, cv::LINE_AA);
    cv::putText(frame, "acquire_policy: " + policy_name,
                cv::Point(20, 155), cv::FONT_HERSHEY_SIMPLEX, 0.62,
                color, 2, cv::LINE_AA);
    if (ekf_state.has_value()) {
        cv::putText(frame, "EKF STATE: " + *ekf_state,
                    cv::Point(20, 185), cv::FONT_HERSHEY_SIMPLEX, 0.62,
...
const char* TargetManager::stateName(TargetState state)
{
    switch (state) {
        ...
        case TargetState::DETECTING: return "DETECTING";
        case TargetState::TRACKING: return "TRACKING";
        case TargetState::TEMP_LOST: return "TEMP_LOST";
...
{
    switch (state) {
        case TargetState::LOST: return cv::Scalar(0, 0, 255);
        case TargetState::DETECTING: return cv::Scalar(0, 255, 255);
        case TargetState::TRACKING: return cv::Scalar(0, 255, 0);
        case TargetState::TEMP_LOST: return cv::Scalar(0, 165, 255);
        case TargetState::SWITCHING: return cv::Scalar(255, 0, 255);
    }
    return cv::Scalar(255, 255, 255);
}

bool TargetManager::allowsFireControl(TargetState state)
{
    return state == TargetState::TRACKING;
}
    ...
    ArmorType::ArmorType acquire_policy) const
{
    ...
    const cv::Point2f image_center(
        static_cast<float>(frame_size.width) / 2.0F,
        static_cast<float>(frame_size.height) / 2.0F);

    for (const ArmorResult& candidate : measurements) {
        if (!isConcreteTargetType(candidate.number) ||
            !candidate.solve_armor_result.valid) {
            continue;
        }

        if (acquire_policy != ArmorType::Nearest &&
            acquire_policy != ArmorType::Middle &&
            candidate.number != static_cast<int>(acquire_policy)) {
            continue;
        }

        double score = 0.0;
        if (acquire_policy == ArmorType::Middle) {
            score = cv::norm(candidate.center - image_center);
...
std::vector<ArmorResult> TargetManager::candidatesForTarget(
    const std::vector<ArmorResult>& measurements) const
{
    ...
    for (const ArmorResult& measurement : measurements) {
        if (measurement.number == target_number &&
            measurement.solve_armor_result.valid) {
...
{
    if (status_.target_type == ArmorType::Outpost) {
        return outpost_max_temp_lost_frames_;
    }
...
{
    if (status_.target_type.has_value()) {
        update.released_target = status_.target_type;
    }
    clearTargetState();
}
...
{
    status_.state = TargetState::LOST;
    status_.target_type.reset();
    status_.detect_count = 0;
    status_.temp_lost_count = 0;
}
