...
struct GeometryDebug {
    ...
    double joint_minus_baseline_ground_range_m = std::numeric_limits<double>::quiet_NaN();
    double center_separation_m = std::numeric_limits<double>::quiet_NaN();
...
struct EKF_fire_result_t { bool aim_center; bool fire; };
...
public:
    ...
    : config_file_ptr(config_file_ptr), node_start_time(node_start_time),
      ...
      rest_frame_(rest_frame_), fps_counter(fps_counter), armor_class(armor_class) {
        ...
        if (fix_bullet_velocity >= 0.0f) bullet_velocity_ = fix_bullet_velocity;
        else bullet_velocity_ = (*config_file_ptr)["bullet_velocity_"].as<float>();
        ...
        const bool show_windows = (*config_file_ptr)["SHOW_WINDOWS"]
            ? (*config_file_ptr)["SHOW_WINDOWS"].as<bool>() : false;
        oscilloscope_common_ = std::make_shared<Oscilloscope>(
            640, 480, "Common Debug Oscilloscope " + std::to_string(armor_class), 2,
            cv::Scalar(0,0,0), cv::Scalar(0,255,0), show_windows);
        ...
        const YAML::Node comparison = (*config_file_ptr)["superpower_ekf"]["comparison"];
        comparison_enabled_ = comparison && comparison["enabled"] && comparison["enabled"].as<bool>();

        // Register the shared world/camera transform for the runtime EKF facade.
        SuperPowerPredictor::setRuntimeRestFrame(rest_frame_);
    }

    PredictorResult step(std::vector<ArmorResult>& classifyResults,
        const std::vector<JointEkfTrackPair>& joint_pairs,
        cv::Mat& frame, double frame_timestamp_s);
...
private:
    ...
    float last_pitch_rad_delayed_=0, last_yaw_rad_delayed_=0, total_yaw_rad_delayed_=0;
    float yaw_rad_to_x_pixel_ratio, pitch_rad_to_y_pixel_ratio;
    ...
    float latest_armor_distance=1e10;
    float pre_predict_time, pre_predict_time_not_aim, choose_armor_yaw_bias, extra_predict_time;
};
